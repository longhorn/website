...
As a consequence, Longhorn engines lose the connection to the replicas making the volume unstable.
...
The instance manager pod is a critical component responsible for managing the engine and replica processes of the volume. If the instance manager pod becomes unstable and crashes or restarts unexpectedly, the volumes also become unstable.
...
One potential cause of instability is an excessive number of replica or engine processes running within the pod. This can overload the server and prevent it from responding to the liveness probe in time.
...
When you upgrade Longhorn to a new version, a new instance manager with an updated image and engine image is created.
...
Longhorn does not restart instance manager pods while volumes are still attached. It waits until all volumes are detached.
