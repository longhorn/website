...
Cluster API (CAPI) performs rolling node replacement by provisioning a new Machine CR and then deleting the old one. If Longhorn has replicas or backing images scheduled on the node being removed, those resources are lost abruptly when the node terminates. This leaves affected volumes in a degraded state until Longhorn can rebuild replicas on remaining nodes.
...
- The volumes using the node being replaced must have a replica count greater than 1.
- At least one other schedulable node with sufficient disk space must be available to receive the migrated replicas and backing images.
...
Eviction is complete when every disk reports `scheduledReplicas: 0` and `scheduledBackingImages: 0`. Once confirmed, the node is safe to remove and CAPI can proceed with Machine deletion.
...
3. **Evict the Longhorn node**: connect to the workload cluster's Longhorn API and disable scheduling and request eviction on the corresponding Longhorn Node CR.
4. **Wait for eviction to complete**: poll the node status until all disks report zero scheduled replicas and backing images.
