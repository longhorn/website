...
However, instead of validating LUKS encryption properly, it mistakenly checks the mapped device's filesystem type. This leads to a failure in the resize operation, causing the PVC to remain in a pending state, even though the expansion appears complete in the UI.
...
2. Upgrade the share manager. There are two options to take effective on existing RWX volumes:
    - Option A: Scale down the workload. After the volume is detached, scale the workload back.
    - Option B: Edit the share manager CR to update the image:
         ```
