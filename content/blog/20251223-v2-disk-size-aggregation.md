...
Because Concat does not interleave I/O across disks, it behaves similarly to a single raw device. For sequential workloads, throughput can be significantly lower than that of a single disk. 
...
| **Sequential Throughput**            | Very high (striping)                                       | Significantly lower than a single disk  | Very high (striping)                            |
...
| **SPDK RAID Concat**       | 3,002 / 3,586              | 11,939 / 12,049                           | 636,214 / 941,915                |
...
| **SPDK RAID Concat**       | 9,017 / 3,409              | 36,013 / 12,215                           | 642,653 / 1,075,667              |
...
1. **Single-disk vs. SPDK Concat**
   ...
   Single-disk configurations and `SPDK Concat` show similar random read and write performance because each I/O request is handled by exactly one underlying device. However, `SPDK Concat` delivers extremely poor sequential throughput—often significantly worse than a single raw disk. This behavior is caused by the lack of striping, absence of read-ahead, and limited parallel I/O dispatch in the Concat implementation.
...
- `SPDK Concat` preserves capacity but does not provide parallel I/O and can deliver significantly lower sequential throughput than a single raw disk.
...
For these reasons, **Linux Kernel RAID** remains the recommended approach for disk size aggregation in Longhorn v2.
