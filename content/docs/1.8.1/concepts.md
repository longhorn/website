...
For V1 volumes, Longhorn relies on iSCSI, which may require additional node setup, including the installation of `open-iscsi` or `iscsiadm`, depending on the distribution. V2 volumes, on the other hand, have different prerequisites, including specific kernel modules like `vfio_pci` and `uio_pci_generic`, `nvme_tcp` for the NVMe-TCP frontend, `ublk` for the UBLK frontend, and support for huge pages.
