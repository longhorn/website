...
To upgrade with Fleet, update the value of `helm.version` in `fleet.yaml` of your GitOps repository:

```
helm:
  repo: https://charts.longhorn.io
  chart: longhorn
  version: v1.6.0 # Replace with the Longhorn version you'd like to upgrade to
  releaseName: longhorn
```

#### Upgrade with Flux

To upgrade with Flux, update the value of `spec.chart.spec.version` in the `HelmRelease` yaml file of your GitOps repository:
...
To upgrade with Argo CD, update the value of `targetRevision` in the `Application` yaml file of your GitOps repository:
...
spec:
  ...
  sources:
    - chart: longhorn
      ...
      targetRevision: v1.6.0 # Replace with the Longhorn version you'd like to upgrade to
```
