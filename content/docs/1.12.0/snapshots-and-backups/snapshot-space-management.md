...
**snapshot-warning-threshold**: The warning threshold for the count-based `TooManySnapshots` condition.
...
The effective threshold is `min(snapshot-warning-threshold, SnapshotMaxCount)`, where `SnapshotMaxCount` is the volume-specific hard limit (populated from `snapshot-max-count` when not explicitly set). This ensures that the warning does not trigger above the hard limit.

For example, with the default settings (`snapshot-warning-threshold` = 100, `snapshot-max-count` = 250), the condition is triggered when a volume reaches 100 snapshots, well before the hard limit is reached. If you lower `SnapshotMaxCount` on a specific volume to 50, the effective threshold becomes 50.
