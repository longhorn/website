...
> **Note:** If Kubernetes has a PriorityClass set as the cluster default, it can lead to issues with Longhorn pods restarting unexpectedly. If a default PriorityClass exists, it is recommended to set the Longhorn setting `PriorityClass` to match the cluster default as part of the installation, or at least before any volumes are created.
