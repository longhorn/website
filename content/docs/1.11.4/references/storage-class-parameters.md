...
When using `strictTopology`, `WaitForFirstConsumer` is strongly recommended so the volume's PV `nodeAffinity` follows the scheduled node of the first consumer pod.
...
> **Note**: For volumes with `dataLocality: strict-local`, `allowedTopologies` is not written to the PV. Longhorn ignores the CSI topology for these volumes and places the data directly on the workload node.
...
This parameter controls only Kubernetes PV `nodeAffinity`. It does not persist a Longhorn replica placement constraint.
