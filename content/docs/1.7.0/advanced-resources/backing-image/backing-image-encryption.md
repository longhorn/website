...
You can clone a backing image using YAML code.
...
> - `encryption`: Set the value to `ignore` to directly clone the backing image
...
- When encrypting a qcow2 image, Longhorn first creates a raw image from the qcow2 image and then encrypts it. The resulting encrypted raw image temporarily consumes extra space during cloning.
